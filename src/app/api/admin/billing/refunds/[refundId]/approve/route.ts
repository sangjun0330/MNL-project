...
import { approveRefundRequest } from "@/lib/server/billingStore";
...
function parseErr(error: any): { status: number; message: string } {
  const message = String(error?.message ?? "approve_refund_failed");
  ...
  return { status: 500, message: "approve_refund_failed" };
}
...
export async function POST(req: Request, ctx: any) {
  ...
  try {
    const request = await approveRefundRequest({
      id: refundId,
