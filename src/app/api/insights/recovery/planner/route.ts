    ...
    try {
      ...
      if (cachedRecoveryIsCurrent) {
        ...
        try {
          ...
          throw new Error(
            typeof explanationError?.message === "string" && explanationError.message.trim()
              ? explanationError.message.trim()
              : "ai_recovery_explanation_failed"
          );
        }
      }
