    ...
    <div className={cn("space-y-4", className)}>
      {showSummary ? (
        <DetailSummaryCard
          ...
          summary={(
            <>
              <span className="font-bold">{t("즉시 실행 오더")}</span> · {ordersSummary.headline}
            </>
          )}
          detail={t("작은 오더부터 실행하면 회복 효율이 올라갑니다.")}
          chips={
            showShift ? (
              <>
                <DetailChip color={DETAIL_ACCENTS.navy}>{shiftKo(todayShift)}</DetailChip>
                <DetailChip color={DETAIL_ACCENTS.navy}>
                  {t("내일")} {shiftKo(nextDuty)}
                </DetailChip>
              </>
            ) : null
          }
        ...
        <div className="space-y-3 px-5 pb-5 pt-4">
          {orderOneLiners.length === 0 ? (
            <div className="rounded-2xl border border-ios-sep bg-ios-bg p-3 text-[12.5px] text-ios-muted">
              {t("데이터가 부족합니다. 수면/카페인/기분 중 1개만 입력해도 추천이 더 정교해집니다.")}
            </div>
          ) : (
              ...
              <div key={o.key} className="rounded-2xl border border-ios-sep bg-white/90 p-4 shadow-apple-sm">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <DetailChip color={DETAIL_ACCENTS.navy}>{t("오더 {rank}", { rank: o.rank })}</DetailChip>
      ...
      <DetailCard>
        <div className="px-5 pt-5">
          ...
          <div className="mt-1 text-[18px] font-bold tracking-[-0.01em] text-ios-text">{t("맞춤 회복 오더")}</div>
        </div>
        <div className="space-y-3 px-5 pb-5 pt-4">
          {orders.length === 0 ? (
            <div className="rounded-2xl border border-ios-sep bg-ios-bg p-3 text-[12.5px] text-ios-muted">
              ...
              <div key={o.key} className="rounded-2xl border border-ios-sep bg-white/90 p-4 shadow-apple-sm">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <DetailChip color={DETAIL_ACCENTS.navy}>{t("오더 {rank}", { rank: o.rank })}</DetailChip>
                      <div className="truncate text-[13px] font-semibold">{t("드라이버 · {label}", { label: o.label })}</div>
                      <div className="text-[12px] text-ios-muted">{pct(o.pct)}</div>
                    </div>
                  </div>
                  {showShift ? (
