        ...
        <>
          {accessState === "granted" ? (
            <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
              <section className="rounded-[30px] border border-white/80 bg-white/92 p-5 shadow-[0_18px_50px_rgba(17,41,75,0.07)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-[18px] font-bold tracking-[-0.02em] text-ios-text">템플릿 목록</div>
                  ...
                  <button
                    ...
                    className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[color:var(--rnest-accent-border)] bg-[color:var(--rnest-accent-soft)] text-[color:var(--rnest-accent)]"
                  >
                    ...
                    return (
                      <button
                        ...
                        className={`w-full rounded-[22px] border px-4 py-3 text-left transition ${
                          selected
                            ? "border-[color:var(--rnest-accent-border)] bg-[color:var(--rnest-accent-soft)] shadow-[0_10px_30px_rgba(167,139,250,0.12)]"
                            : "border-[#e5ecf4] bg-[#fbfcfe] hover:border-[color:var(--rnest-accent-border)]"
                        }`}
                      >
                        <div className="flex items-center gap-2">
                          <span className="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-white text-[13px] font-semibold text-[color:var(--rnest-accent)] shadow-[inset_0_0_0_1px_rgba(167,139,250,0.18)]">
                            {template.icon}
                          </span>
                          <div className="min-w-0 flex-1">
                            <div className="truncate text-[14px] font-semibold text-ios-text">{template.label}</div>
                            <div className="truncate text-[12px] text-ios-sub">{memoTemplateToPreviewText(template)}</div>
                          </div>
                        </div>
                      </button>
                    );
              ...
              <section className="rounded-[34px] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.97),rgba(244,248,252,0.95))] p-6 shadow-[0_22px_70px_rgba(17,41,75,0.08)]">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-3 text-[24px] font-bold tracking-[-0.03em] text-ios-text">
                      {selectedTemplate?.label ?? "템플릿을 선택하세요"}
                    </div>
                    <div className="mt-2 text-[13px] leading-6 text-ios-sub">
                      메타데이터는 여기서 편집하고, 실제 본문 구조는 기존 메모를 골라 템플릿으로 덮어씁니다.
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl border border-[#d9e2ee] bg-white px-3 py-2 text-[12px] font-semibold text-[#41556f] disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl border border-[#f0d9d9] bg-white px-3 py-2 text-[12px] font-semibold text-[#b04a4a] disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl bg-[color:var(--rnest-accent)] px-4 py-2 text-[12px] font-semibold text-white shadow-[0_16px_36px_rgba(167,139,250,0.28)] disabled:opacity-60"
                    >
                ...
                {loading ? <div className="mt-4 text-[12px] text-ios-muted">템플릿과 메모 원본을 불러오는 중...</div> : null}
                {error ? <div className="mt-4 text-[12px] text-red-600">{error}</div> : null}

                {selectedTemplate ? (
                  <>
                    <Separator className="my-6 bg-[#e5ecf4]" />

                    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
                      <div className="space-y-5">
                        <div className="rounded-[28px] border border-white/80 bg-white/92 p-5 shadow-[0_12px_40px_rgba(17,41,75,0.06)]">
                          <div className="text-[15px] font-bold text-ios-text">기본 정보</div>
                          <div className="mt-4 grid gap-4 md:grid-cols-2">
                            <label className="block">
                              ...
                              <input
                                ...
                                placeholder="템플릿 이름"
                                className="h-11 w-full rounded-2xl border border-[#dbe4ef] bg-[#fbfcfe] px-4 text-[14px] text-ios-text outline-none focus:border-[color:var(--rnest-accent-border)]"
                              />
                            </label>
                            ...
                            <label className="block md:col-span-2">
                              ...
                              <textarea
                                ...
                                placeholder="템플릿 설명"
                                className="min-h-[104px] w-full rounded-[22px] border border-[#dbe4ef] bg-[#fbfcfe] px-4 py-3 text-[14px] text-ios-text outline-none focus:border-[color:var(--rnest-accent-border)]"
                              />
                            </label>
                        ...
                        <div className="rounded-[28px] border border-white/80 bg-white/92 p-5 shadow-[0_12px_40px_rgba(17,41,75,0.06)]">
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            <div>
                              <div className="text-[15px] font-bold text-ios-text">메모에서 템플릿 가져오기</div>
                              <div className="mt-1 text-[12px] text-ios-sub">
                                실제 블록 구성은 메모 페이지에서 다듬고 여기서 템플릿으로 덮어씁니다.
                              </div>
                            </div>
                            <button
                              type="button"
                              onClick={() => void load()}
                              className="inline-flex items-center gap-2 rounded-2xl border border-[#d9e2ee] bg-white px-3 py-2 text-[12px] font-semibold text-[#41556f]"
                            >
                          ...
                          <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
                            ...
                            <button
                              ...
                              className="rounded-2xl bg-[color:var(--rnest-accent)] px-4 py-2 text-[12px] font-semibold text-white shadow-[0_16px_36px_rgba(167,139,250,0.22)] disabled:opacity-60"
                            >
                              선택 메모로 덮어쓰기
                            </button>
                          </div>
                          ...
                          {selectedSourceDoc ? (
                            <div className="mt-4 rounded-[22px] border border-[#e7edf5] bg-[#fbfcfe] p-4">
                              ...
                              <div className="mt-3 text-[12px] leading-6 text-ios-sub">
                      ...
                      <div className="space-y-5">
                        <div className="rounded-[28px] border border-white/80 bg-white/92 p-5 shadow-[0_12px_40px_rgba(17,41,75,0.06)]">
                          <div className="text-[15px] font-bold text-ios-text">미리보기</div>
                          <div className="mt-4 rounded-[24px] border border-[#e7edf5] bg-[#fbfcfe] p-4">
                            <div className="inline-flex rounded-full bg-[color:var(--rnest-accent-soft)] px-3 py-1 text-[12px] font-semibold text-[color:var(--rnest-accent)]">
                              {selectedTemplate.icon}
                            </div>
                            <div className="mt-4 text-[20px] font-bold tracking-[-0.03em] text-ios-text">
                              {selectedTemplate.title || "제목 없음"}
                            </div>
                            <div className="mt-2 text-[13px] leading-6 text-ios-sub">{selectedTemplate.description}</div>
                        ...
                        <div className="rounded-[28px] border border-white/80 bg-white/92 p-5 shadow-[0_12px_40px_rgba(17,41,75,0.06)]">
                          <div className="text-[15px] font-bold text-ios-text">운영 메모</div>
                          <div className="mt-3 space-y-2 text-[12px] leading-6 text-ios-sub">
                            <p>새 페이지 생성 팝업에는 여기서 저장한 순서대로 템플릿이 노출됩니다.</p>
                            <p>잠금 메모는 본문을 읽을 수 없어서 템플릿 원본으로 사용할 수 없습니다.</p>
                            <p>이미지/파일 첨부 블록은 템플릿 저장 시 자리표시 텍스트로 변환됩니다.</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-6 text-[13px] text-ios-sub">왼쪽에서 템플릿을 선택하거나 새 템플릿을 추가하세요.</div>
                )}
              </section>
            </div>
          ) : (
