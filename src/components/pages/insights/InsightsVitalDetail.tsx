    ...
    >
      ...
      <DetailCard className="mt-4 overflow-hidden" style={{ backgroundImage: DETAIL_GRADIENTS.mint }}>
        <div className="px-5 pt-5">
          ...
          <div className="mt-1 text-[18px] font-bold tracking-[-0.01em] text-ios-text">오늘 상태 디테일</div>
        </div>
        <div className="px-5 pb-5 pt-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-ios-sep bg-white/90 p-4">
              <div className="text-[12px] font-semibold text-ios-sub">바이탈(Vital)</div>
            ...
            <div className="rounded-2xl border border-ios-sep bg-white/90 p-4">
              <div className="text-[12px] font-semibold text-ios-sub">수면부채</div>
              <div className="mt-2 text-[26px] font-extrabold">
                {debt}
                <span className="ml-1 text-[14px] font-semibold text-ios-muted">h</span>
              </div>
              <div className="mt-1 text-[12.5px] text-ios-sub">부채가 높을수록 듀티 피로/실수 리스크가 증가합니다.</div>
            </div>

            <div className="rounded-2xl border border-ios-sep bg-white/90 p-4">
              <div className="text-[12px] font-semibold text-ios-sub">바디 배터리</div>
            ...
            <div className="rounded-2xl border border-ios-sep bg-white/90 p-4">
              <div className="text-[12px] font-semibold text-ios-sub">멘탈 배터리</div>
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-ios-sep bg-white/90 p-4">
              <div className="text-[12px] font-semibold text-ios-sub">리듬/수면/카페인</div>
              <div className="mt-2 text-[15px] font-semibold text-ios-text">CSI {pct(csi)} · SRI {pct(sri)} · CIF {pct(cif)}</div>
              <div className="mt-1 text-[12.5px] text-ios-sub">CSI는 리듬 부담, SRI는 회복, CIF는 카페인 영향(낮을수록 방해↑) 지표입니다.</div>
            </div>
            <div className="rounded-2xl border border-ios-sep bg-white/90 p-4">
              <div className="text-[12px] font-semibold text-ios-sub">연속 나이트</div>
              <div className="mt-2 text-[26px] font-extrabold">{night}</div>
              <div className="mt-1 text-[12.5px] text-ios-sub">연속 나이트가 쌓이면 회복 우선순위를 더 높입니다.</div>
            </div>
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {top3?.map((t) => (
              <DetailChip key={t.key} color={DETAIL_ACCENTS.mint}>
                드라이버 · {FACTOR_LABEL_KO[t.key as FactorKey]} {pct(t.pct * 100)}
              </DetailChip>
            ))}
          </div>
        </div>
