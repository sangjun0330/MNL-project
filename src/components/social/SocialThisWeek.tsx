...
function ShiftBadge({ shift }: { shift: string | null }) {
  if (!shift) {
    return <span className="text-[10px] text-ios-muted">—</span>;
  }
  return (
    <span
      className={`rounded-lg px-1.5 py-0.5 text-[10px] font-bold ${SHIFT_COLORS[shift] ?? "bg-ios-bg text-ios-text"}`}
    >
      {shift}
    </span>
  );
    ...
    <div className="rounded-apple border border-ios-sep bg-white shadow-apple px-4 py-3">
      ...
      >
        ...
        <div className="flex flex-col items-center">
          <span className="text-[10px] font-semibold text-ios-muted truncate">나</span>
        </div>
        {/* 친구들 */}
        {visibleFriends.map((f) => (
          <div key={f.userId} className="flex flex-col items-center">
            <span className="text-[18px] leading-none">{f.avatarEmoji || "🐧"}</span>
            <span className="text-[9px] text-ios-muted truncate mt-0.5 max-w-[40px] text-center leading-tight">
              {f.nickname || "친구"}
            </span>
          </div>
        ))}
          ...
          return (
            ...
            >
              ...
              <div className="flex items-baseline gap-1">
                <span className={`text-[11px] font-semibold ${weekday === 0 ? "text-red-500" : weekday === 6 ? "text-blue-500" : "text-ios-text"}`}>
                  {label}
                </span>
                <span className={`text-[9px] ${weekday === 0 ? "text-red-400" : weekday === 6 ? "text-blue-400" : "text-ios-muted"}`}>
                  {WEEKDAY_KO[weekday]}
                </span>
              </div>
      ...
      {friends.length > 4 && (
        <p className="mt-2 text-[11px] text-ios-muted text-center">
