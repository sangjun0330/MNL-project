...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  if (status === "loading") {
    return (
      <div className="rounded-apple border border-ios-sep bg-white p-5 shadow-apple">
        <div className="text-[14px] font-semibold">로그인 확인 중…</div>
        <div className="mt-1 text-[12.5px] text-ios-muted">잠시만 기다려 주세요.</div>
      </div>
    );
  }
  ...
  if (status !== "authenticated" && !isAllowed) {
    return (
      ...
      >
        <div className="text-[15px] font-semibold">로그인이 필요합니다</div>
