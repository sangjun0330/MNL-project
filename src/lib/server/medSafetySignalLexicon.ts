...
function adjustIntentByQuestionStructure(scores: IntentScoreMap, query: string): IntentScoreMap {
  ...
  if (countPatternHits(tail, [...NUMERIC_PATTERNS, ...INTERPRET_PATTERNS, ...THRESHOLD_PATTERNS]) > 0) {
    adjusted.numeric += 2;
  }
...
function inferIntentScores(query: string): IntentScoreMap {
  return adjustIntentByQuestionStructure(buildBaseIntentScores(query), query);
}
...
export function buildQuestionSignals(query: string, options: { hasImage?: boolean } = {}): MedSafetyQuestionSignals {
  ...
  return {
    ...
    mixedIntent:
      topIntent.topScore > 0 &&
      topIntent.secondScore > 0 &&
