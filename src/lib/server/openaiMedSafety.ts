...
function buildDeveloperPrompt(locale: "ko" | "en", intent: QueryIntent) {
  if (intent === "medication") {
    return [
      ...
      "- 입력이 한국어/영어/약어/오타/붙여쓰기/제형·용량 포함이어도 의도를 복원해 “정규화→후보→확신도”로 처리한다.",
      "- 절대 사실을 지어내지 마라. 확신이 낮으면 임상 세부를 생성하지 말고 NOT_FOUND로 종료한다.",
      "- 출력 내용은 의료학·의학·간호학적으로 신뢰 가능한 데이터와 정보만으로 구성한다.",
      "",
      "핵심 원칙(환각 방지):",
      ...
      "- 숫자/단위(mg, mcg, g, mL, IU, mEq, %, 시간표현 8시간/12시간 등)",
      "- 제형/서방표현(정, 캡슐, 시럽, 주, 연고, 패치, 흡입, 서방, 이알, ER, SR, CR 등)",
      "- 조합 표기(+, /)는 분리 후보로 취급",
      "- 붙여쓰기(예: “써스펜8시간이알서”)는 토큰을 분리해 core를 추출한다.",
      "- 한글 음역(예: “노르에피”, “반코”, “바코마이신”)은 가능한 표준명 후보로 확장한다.",
      "",
      "2. 후보 생성(CANDIDATE GENERATION)",
      "- core를 기반으로 후보를 최대 3개 생성한다.",
      "- 후보는 가능한 경우 “성분명(일반명) 우선”, 다음 “대표 제품명/브랜드명” 순으로 제시한다.",
      ...
      "- 환자 안전에 직결되는 High-alert/LASA 가능성이 보이면 강하게 경고한다.",
    ].join("\n");
  }
  ...
  return [
    ...
    "Use only medically and nursing-reliable data and information in outputs.",
