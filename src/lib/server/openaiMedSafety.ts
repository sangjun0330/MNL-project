...
function buildLegacyDeveloperPrompt(locale: "ko" | "en") {
  if (locale === "en") {
    return [
      ...
      "- Default to at most 2 bullets per section, and allow a 3rd bullet only when risk, reporting, or exception boundaries would otherwise be underspecified.",
      "- Keep each bullet to one sentence by default, stretching to 2 sentences only when that prevents clinically important loss.",
      "- Do not repeat the same warning, caveat, or rationale across multiple sections; keep it in the single best-fitting section.",
      "- Keep background explanation brief and only as far as it changes the next action, without trimming essential clinical judgment, safety warnings, stop-report thresholds, or required exceptions.",
      "- In low or medium-risk cases, a lower-priority teaching or comparison section may be absorbed into a nearby core section instead of becoming a standalone section. Do not merge away the conclusion, immediate actions, safety warnings, reporting content, or high-risk exception boundaries.",
      ...
      "- Keep enough detail to avoid truncating clinically important reasoning while making each section slightly tighter than usual.",
      "",
      "[LANGUAGE_OVERRIDE]",
  ...
  return [
    ...
    "- 각 소제목 아래 첫 줄은 해당 섹션의 핵심을 한 문장으로 요약하고, 필요하면 그 아래에 \"* \" 또는 \"- \" bullet로 세부 내용을 둔다.",
    "- 각 섹션은 기본 2개 이내 bullet로 정리하고, 위험도·보고 필요·예외 경계 때문에 부족해질 때만 3번째 bullet을 허용한다.",
    "- 각 bullet은 기본 1문장으로 쓰고, 임상적으로 중요한 내용이 빠질 때만 2문장까지 늘린다.",
    "- 같은 경고, 주의 문구, 근거를 여러 섹션에 반복하지 말고 가장 적절한 섹션 1곳에만 둔다.",
    "- 배경 설명은 실제 행동을 바꾸는 범위까지만 짧게 두되, 중요한 임상 판단, 안전 경고, 중단/보고 기준, 필수 예외는 줄이지 않는다.",
    "- 저위험/중등도 상황에서는 교육성 설명이나 부가 비교 설명을 인접한 핵심 섹션에 짧게 흡수할 수 있다. 다만 결론, 즉시 행동, 안전 경고, 보고 기준, 고위험 예외 내용은 독립성을 유지한다.",
    '- 소제목은 내용에 맞는 구체 제목을 사용한다. "요약", "상세"처럼 두루뭉술한 제목은 쓰지 않는다.',
    "- 마크다운 강조(**, __, 백틱, ##, 표, 코드블록)는 쓰지 않는다.",
    "- 임상적으로 필요한 설명은 중간에 잘리지 않도록 충분히 끝까지 쓰되, 각 섹션은 지금보다 약간 더 압축되게 쓴다.",
  ].join("\n");
}
...
export async function translateMedSafetyToEnglish(input: {
  ...
  const maxOutputTokens = Math.max(8000, Math.min(28000, resolveMaxOutputTokens() + 4000));
